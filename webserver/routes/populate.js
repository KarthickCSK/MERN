...
const shopList = require('../models/vendorshopmodel.js')
...
router.get('/showproduct', function(req, res, next) {
 productInformation.find({}).populate(' ').exec((err,data)=>{
  	res.send(data)
  }) 

  
});
router.get('/showVendor', function(req, res, next) {

 vendorInformation.find({}).populate('').exec((err,data)=>{
    res.send(data)
  })

  
});
router.get('/showShop', function(req, res, next) {
 shopList.find({}).populate('').exec((err,data)=>{
    res.send(data)
  })
