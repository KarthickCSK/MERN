...
router.get('/profile', function(req, res) {
    userdata.findOne({
        'local.email': req.user.local.email // || ('facebook.email': req.user.facebook.email )|| ('google.email': req.user.google.email)
...
router.put('/updatepassword/:email', function(req, res)
{
  ...
  userdata.findOne({'local.email': req.params.email}, function(err, profile)
{
  if(err) throw err;
  else{
